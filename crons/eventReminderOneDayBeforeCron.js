...
cron.schedule("0 4 * * *", function () {
    //every day at 04 AM
    console.log("eventReminderOneDayBeforeCron working...");
    eventReminderOneDayBeforeNotifications();
});

// eventReminderOneDayBeforeNotifications();

async function eventReminderOneDayBeforeNotifications() {
    try {
        ...
        let eventData = await Events.find({ $and:[{startDate: { $gte: startDate }},{startDate: { $lte: endDate }}], expired: false, isActive:true, isDeleted: false },{ name:1, startDate: 1, endDate: 1, managedBy:1 }).lean();
        ...
        for (let i = 0; i < allUsers.length; i++) {
            ...
            if(user._id && user.event && user.event._id){
                let notificationObj = {
                    ...
                    title: `${user.event.name} event starts tomorrow!`,
                    message: `Just a heads-up! ${user.event.name} kicks off tomorrow. Don't forget to check in!`,
                    email: user.email,
                    userId: user._id,
                    isSent: false,
                    isRead: false,
